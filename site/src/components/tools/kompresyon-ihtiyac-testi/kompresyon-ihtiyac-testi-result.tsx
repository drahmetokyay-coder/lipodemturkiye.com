...
import { Shirt, AlertTriangle, CheckCircle2, Download, Printer, RotateCcw } from "lucide-react";
import { RESULT_CONTENT, type TestResult } from "@/data/tests/kompresyon-ihtiyac-testi";
...
const FLAG_NOTES: Record<string, string> = {
  "surgical-candidate":
    "Evre 3 lipödem işaretlediniz. Konservatif tedavi yanında cerrahi adaylık değerlendirmesini de gündeminize alın — Evre 3 hastaları liposuction sonrası yaşam kalitesinde en belirgin iyileşmeyi yaşıyor.",
};

const BAND_BADGE_CLASS: Record<TestResult["band"], string> = {
  LOW: "bg-[#E8F5F0] text-[#2D8B73]",
  MODERATE: "bg-[#EEF1F7] text-[#6B7B99]",
  HIGH: "bg-[#FEF3E6] text-[#C46B3D]",
};

export function KompresyonIhtiyacTestiResult({ result, onRestart }: Props) {
    ...
    <div className="space-y-6">
      <div className="rounded-3xl border border-stone-100 overflow-hidden shadow-lg bg-white">
        <div className="bg-white p-8 md:p-10 text-center border-b border-stone-100">
          <div className="w-16 h-16 bg-[#EEF1F7] rounded-full flex items-center justify-center mx-auto mb-4">
            <CheckCircle2 className="w-8 h-8 text-[#6B7B99]" />
          </div>
          <h2 className="text-2xl md:text-3xl font-bold text-[#2D3B36] mb-1">Test Tamamlandı</h2>
          <p className="text-[#2D3B36]/50 text-sm">Kompresyon İhtiyaç Skorunuz</p>
        </div>
          ...
          <div className="flex flex-col items-center">
            <div className="relative mb-6">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-xs text-[#2D3B36]/50">ihtiyaç düzeyi</span>
              </div>
            </div>
        ...
        <div className="bg-white p-8 md:p-10">
          ...
          <div className="bg-[#FAF7F2] border-l-4 border-[#6B7B99] p-5 rounded-r-xl mb-6">
            <div className="text-[10px] font-bold tracking-wider text-[#6B7B99] mb-2">
              KLİNİK YORUM
            </div>
            ...
            <ul className="space-y-3">
              {content.steps.map((step, i) => (
                <li key={i} className="flex items-start gap-3">
                  <span className="flex-shrink-0 w-7 h-7 bg-[#EEF1F7] text-[#6B7B99] rounded-full flex items-center justify-center text-sm font-bold mt-0.5">
                    {i + 1}
                  </span>
      ...
      <div className="bg-white rounded-3xl border border-stone-100 shadow-sm p-6 print:hidden">
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            ...
            className="flex-1 inline-flex items-center justify-center gap-2 bg-[#6B7B99] text-white px-5 py-3.5 rounded-2xl font-semibold hover:bg-[#54627F] transition shadow-md"
          >
          ...
          <button
            ...
            className="flex-1 inline-flex items-center justify-center gap-2 bg-white text-[#6B7B99] border border-[#6B7B99] px-5 py-3.5 rounded-2xl font-semibold hover:bg-[#EEF1F7] transition"
          >
      ...
      <div className="p-5 bg-stone-50 rounded-2xl">
        <p className="text-xs text-[#2D3B36]/40 leading-relaxed">
          <strong>Uyarı:</strong> Bu test bir tanı veya reçete aracı değildir. Cornely-Schmeller
          endikasyon algoritması ve CEAP-C basamağından uyarlanmış bir ön değerlendirmedir.
          Kompresyon sınıfı (CCL 1-4) ve ürün tipi yalnızca bireysel bacak ölçümü ile uzman
      ...
      <EmailCaptureModal
        ...
        testSlug="kompresyon-ihtiyac-testi"
        band={result.band}
        score={result.totalScore}
        pdfPayload={{
          testTitle: "Kompresyon İhtiyaç Testi",
          bandLabel: content.title,
          scoreLine: `${result.totalScore} / ${result.maxScore} puan · %${result.percentage} ihtiyaç`,
          percentage: result.percentage,
          clinicalInterpretation: content.clinicalInterpretation,
          steps: content.steps,
          flagNotes,
          filename: "kompresyon-ihtiyac-testi-sonuc",
        }}
      />
    </div>
  );
